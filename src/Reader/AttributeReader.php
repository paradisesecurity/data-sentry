...
/**
 * Modified version of Doctrine AttributeReader
 * https://github.com/doctrine/orm/blob/2.14.x/lib/Doctrine/ORM/Mapping/Driver/AttributeReader.php
 */
class AttributeReader implements AttributeReaderInterface
{
    private string $interface;

    public function __construct(string $interface = AttributeInterface::class)
    {
        if (interface_exists($interface)) {
            $this->interface = $interface;
        }

        $this->interface = AttributeInterface::class;
    }
    ...
    private function convertToAttributeInstances(array $attributes): array
    {
        ...
        foreach ($attributes as $attribute) {
            ...
            assert($instance instanceof $this->interface);
